...
-(void)setup{
    self.timeInterval = 2;
    self.placeholderImage = [UIImage imageNamed:@"last_5"];
    self.noDataPlaceholderImage = [UIImage imageNamed:@"last_5"];
    ...
    [_collectionView registerClass:[ScrollHeadViewCell class] forCellWithReuseIdentifier:Identifier];
    ...
    switch (pageControlPosition) {
        case PageControlPositionCenter:
        {
            ...
            [_pageControl mas_makeConstraints:^(MASConstraintMaker *make) {
                make.centerX.equalTo(_bottomBackView.mas_centerX).with.offset(0);
                make.centerY.equalTo(_bottomBackView.mas_centerY).with.offset(0);
...
{
    ScrollHeadViewCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:Identifier forIndexPath:indexPath];
...
//    if ([urlPath hasPrefix:@"http://"]||[urlPath hasPrefix:@"https://"])
    if(![_noUrl isEqualToString:@"1"])
    {
        [cell.imageView setImageWithURL:[NSURL URLWithString:[NSString stringWithFormat:@"%@%@",primAPI,urlPath]] placeholder:nil options:YYWebImageOptionProgressive completion:^(UIImage * _Nullable image, NSURL * _Nonnull url, YYWebImageFromType from, YYWebImageStage stage, NSError * _Nullable error) {
            //NSLog(@"sdsafdasfsdga");
        }
        ];
    }else{
        cell.imageView.image = [UIImage imageNamed:self.imageUrls[index]];// self.imageUrls[index];
    }
